...
2. **Confirm before coding.** Before implementing any functional change, restate your understanding to the user and get her go-ahead. (Edits to `docs/*.md` usually don't need this — except the cases listed in `~/.claude/docs/maintenance.md` § "Ask the user BEFORE".)
...
After editing JS, run the syntax check in `docs/project-conventions.md` (this machine has no node; the osascript pipeline there is tested and works).

## When to read the other docs

Two layers: `~/.claude/docs/` holds general rules shared across all projects
on this machine (dispatch, judgment, delegation, maintenance); `docs/`
in this repo holds mama-app-specific detail and the concrete values that
plug into those general rules.

| Situation | Read first |
|---|---|
| Any non-trivial change to app behavior | `docs/architecture.md` |
| Spawning a subagent / choosing a model | `~/.claude/docs/dispatch.md`, then `docs/project-conventions.md` for this repo's calibration |
| Unsure whether it's done, whether to ask the user, whether to escalate, or whether you're off track | `~/.claude/docs/judgment.md`, then `docs/project-conventions.md` for the concrete commands/numbers/examples |
| Writing a delegation prompt | `~/.claude/docs/delegation-templates.md`, then `docs/project-conventions.md` for this repo's fill-ins |
| You made a costly mistake, or want to edit these docs | `~/.claude/docs/maintenance.md`, then log it in `docs/lessons.md` |
